...
#### 2/20 [Docker와 VM](https://medium.com/@darkrasid/docker%EC%99%80-vm-d95d60e56fdd)

- vm : 하이퍼바이저라는 가상화 소프트웨어 위에 운영체제(os)를 설치하는 형태. 각 vm은 독립적인 가상 하드웨어를 사용한다.
- 컨테이너 : 호스트 os에서 독립적인 공간을 만들어 운영체제 수준에서 가상화를 수행.

- 장점

  - 가벼움, 적은 용량 : 컨테이너는 호스트의 커널을 공유하고 필요한 라이브러리와 애플리케이션만 패키징하기 떄문에 가상머신에 비해 가볍다.
  - 빠른 속도 : 호스트 커널을 공유하기 때문에 들어온 io가 쉽게 처리돼서 나갈 수 있다.

- 단점
  - 보안 취약 : 호스트 커널을 공유하기 때문에 컨테이너 하나가 뚫리면 호스트 커널에도 영향을 미칠 수 있다.
  - 멀티 OS X: 호스트와 동일한 os만 컨테이너로 실행할 수 있다.
    - ex) 리눅스 머신에서 윈도우 서버를 컨테이너로 올릴 수 없음

#### 2/21 [개발자를 위한 효율적인 MacOS 백업 방법](https://www.letmecompile.com/%ea%b0%9c%eb%b0%9c%ec%9e%90%eb%a5%bc-%ec%9c%84%ed%95%9c-%ed%9a%a8%ec%9c%a8%ec%a0%81%ec%9d%b8-macos-%eb%b0%b1%ec%97%85-%eb%b0%a9%eb%b2%95/)

- appCleaner를 이용하면 앱 바이너리파일, 설정/캐시 파일까지 깨끗하게 삭제됨.
- 타임머신을 이용한 백업
  - 홈 디렉토리의 불필요한 파일 제외하기 (캐시 디렉토리)
  - 프로젝트 불필요한 파일 제거
    - 의존선 모듈 디렉토리들이 프로젝트 내부에 생성됨.
    - asimov 스크립트를 사용하면 홈디렉토리 내에서 지정한 패턴을 찾아 해당 디렉토리 백업 금지 태깅을 할 수 있다. -> 태깅된 파일은 다른곳으로 파일위치를 옮겨도 여전히 백업에서 제외됨
- 수동 백업
  - 필요한 파일들만 복사한 뒤 포맷한 맥에 하나씩 옮겨주는 방법
  - 외장 하드로 파일을 수동으로 복사할 때 파티션 종류에 따라 디렉토리 권한 설정이 꼬일수 있으므로 복사하기 전에 일부 파일을 외장하드에 복사한 후 원래 파일과 백업된 파일의 권한이 동일한지 확인하고 수동백업을 진행하자.
